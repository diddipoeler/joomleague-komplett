<?php
class NextMatchExtJoomleagueConnector extends modNextMatchExtHelper {
	...
	public function buildTeamLinks(& $obj, $nr) {
		...
		$linktext = '';
		$urls = array ();
		if (defined('_MODJLML_JOOMLEAGUE15')) {
			$linkstructure = array (
				'club' => array (
					'view' => 'clubinfo',
					'cid' => $obj->club_id,
		...
		elseif (defined('_MODJLML_JOOMLEAGUE_JC')) {
			$linkstructure = array (
				'club' => array (
					'func' => 'showclubinfo',
					'cid' => $obj->club_id
	...
	public function getDateString() {
		if ($this->params->get('use_offset_matches') == 1) {
...
public function getDateStringNoTime() {
	...
	public function getMatches() {
		$limit = ($this->params->get('limit', 0) > 0) ? $this->params->get('limit', 0) : 1;
		$query = "SELECT m.*,m.id as match_id, t1.id team1_id, t2.id team2_id," . $this->getDateStringNoTime() . " AS match_date_notime,"
		 . $this->getDateString() . " AS match_date,
			NOW() AS currenttime,
			IF ((m.match_result_type > 0), ((p.game_regular_time+(p.game_parts * p.halftime)-p.halftime) + p.add_time), (p.game_regular_time+(p.game_parts * p.halftime)-p.halftime)) AS totaltime,
			IF(
			  (match_date > NOW() AND m.team1_result IS NULL) AND 
			  ((m.team1_result_split IS NULL) OR
			  TRIM(REPLACE(m.team1_result_split, ';', '')) = '')
			  ," . $this->getDateString() . ",'z') AS upcoming,
			  IF(
			  (m.team1_result IS NOT NULL OR " . $this->getDateString() . " < NOW())
			  ," . $this->getDateString() . ",0) AS alreadyplayed,
			  IF 
	(
	  (
	    (
	      DATE_ADD(
	        " . $this->getDateString() . ", INTERVAL 
			    IF
			      (
			        (m.match_result_type > 0), ((p.game_regular_time+(p.game_parts * p.halftime)-p.halftime) + p.add_time), (p.game_regular_time+(p.game_parts * p.halftime)-p.halftime))
			   MINUTE) > NOW()) AND (" . $this->getDateString() . " < NOW()) AND (m.team1_result IS NULL)), " . $this->getDateString() . ", 'z') AS actplaying,
	IF(
			(
			  
			  (m.team1_result IS NULL) AND 
			  (m.team1_result_split IS NOT NULL) AND
			  TRIM(REPLACE(m.team1_result_split, ';', '') != '')
			)
			  ," . $this->getDateString() . ",'z') AS live,
			DATE_SUB(" . $this->getDateString() . ", INTERVAL '90' MINUTE) AS meetingtime,
						DATE_ADD(NOW(), INTERVAL p.serveroffset HOUR) AS local_time,
	                 r.*,
	                 p.name AS pname,
	                 p.current_round,
	                 p.id AS project_id,
	      ...
	      IF(mref.project_referee_id > 0, concat(person.lastname, ', ', person.firstname), '') AS refname,
	                 pg.name AS pg_name, pg.short_name AS pg_shortname
		FROM     #__joomleague_match m
	         INNER JOIN #__joomleague_round r
	         ON       m.round_id = r.id
	         INNER JOIN #__joomleague_project_team pt1
	         ON       pt1.id = m.projectteam1_id
	         INNER JOIN #__joomleague_team t1
	         ON       t1.id = pt1.team_id
	         INNER JOIN #__joomleague_project_team pt2
	         ON       pt2.id = m.projectteam2_id
	         INNER JOIN #__joomleague_team t2
	         ON       t2.id = pt2.team_id
	         INNER JOIN #__joomleague_project p
	         ON       pt1.project_id = p.id
	         LEFT JOIN #__joomleague_match_referee AS mref
	         ON       m.id = mref.match_id
	         LEFT JOIN #__joomleague_project_referee AS pref
	         ON       pref.id = mref.project_referee_id
	         LEFT JOIN #__joomleague_person AS person
	         ON       person.id = pref.person_id
	         AND person.published = 1
	         AND pref.published = 1
	         LEFT JOIN #__joomleague_playground AS pg
	         ON       pg.id = m.playground_id
		  WHERE (";
		$this->buildWhere();
	...
	public function getUsedTeams() {
		$allteamsstr = '';
		$customteamsstring = '';
		$customteams = $this->params->get('teams', '0');
		$ajaxteam = JRequest :: getVar('usedteam', 0, 'default', 'POST');
		$customteams = (is_array($customteams)) ? $customteams : array ($customteams);
		if ($ajaxteam > 0) {
			array_push($customteams, $ajaxteam);
		}
		if (!empty ($customteams) AND $customteams[0] != '0') {
			$this->usedteams[0] = $customteams;
			$customteamsstring = implode(',', $customteams);
			$allteamsstr .= $customteamsstring;
			$customteamsstring = "pt1.team_id IN (" . $customteamsstring . ") 
			                      OR pt2.team_id IN (" . $customteamsstring . ")";
		}
		if ($this->params->get('use_fav', 0) == 0) {
			if (!empty ($customteamsstring)) {
				$this->conditions[] = "(" . $customteamsstring . ")";
			}
		} else {
			...
			if ($fav = $this->getFromDB($query, 'id', 'assc')) {
				...
				foreach ((array) $fav AS $key => $team) {
					...
					if (!empty ($allteamsstr))
						$allteamsstr .= ',';
					$allteamsstr .= $fav_team;
					$cond = "(
					                    ((pt1.team_id IN (" . $fav_team . ") 
					                     OR pt2.team_id IN (" . $fav_team . "))
					                    AND ((pt1.project_id = '" . $key . "')  
					                    	 OR pt2.project_id = '" . $key . "')))";
					if (!empty ($customteamsstring)) {
						$cond .= " OR (" . $customteamsstring . ")";
					}
					$tmp[] = $cond;
				}
				$this->conditions[] = "(" . implode(' OR ', $tmp) . ")";
			}
		}
	}
