...
class modNextMatchExtHelper {
	...
	public function getFromDB($query, $key = "", $type = "objlist") {
		...
		if (defined('_MODJLML_JOOMLEAGUE_JC')) {
			$query = str_replace('#__', $this->params->get('prefix', 'your_'), $query);
		}
		$db->setQuery($query);
		...
		foreach ((array) $matches AS $key => $match) {
			...
			if ($this->params->get('show_status_notice') == 1) {
				...
				$rows[$match->match_id]['type'] = 'live';
					...
					elseif ($match->alreadyplayed != 0) $rows[$match->match_id]['type'] = 'alreadyplayed';
					elseif ($match->upcoming != 'z') $rows[$match->match_id]['type'] = 'upcoming';
				}
			}
			$rows[$match->match_id]['date'] = JHTML :: _('date', $match->match_date, $this->params->get('dateformat'), 0);
			if ($useicons)
			$rows[$match->match_id]['date'] = JHTML :: _('image', $this->iconpath . 'date.png', 'Datum', array (
				...
				' ' . $rows[$match->match_id]['date'];
					$rows[$match->match_id]['time'] = JHTML :: _('date', $match->match_date, $this->params->get('timeformat'), 0);
					if ($useicons)
				...
				' ' . $rows[$match->match_id]['time'];
					if (isset ($match->meeting)) {
						$rows[$match->match_id]['meeting'] = JHTML :: _('date', $match->meetingtime, $this->params->get('timeformat'), 0);
						if ($useicons)
