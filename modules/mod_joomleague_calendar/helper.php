...
class modJLCalendarHelper
{
	...
	function showCal(&$params,$year,$month,$ajax=0,$modid) //this function returns the html of the calendar for a given month
	{
		...
		$article= $language->_('JLCVALUEMATCH');
		$articles= $language->_('JLCVALUEMATCHES'); //this strings are used for the titles of the links
		$article2= $language->_('JLCMATCHTHISDAY');
...
class JLCalendar extends PHPCalendar
{
	...
	function matches_output($month, $year) {
		...
		$articles= $language->_('JLCVALUEMATCHES'); //this strings are used for the titles of the links
		$article2= $language->_('JLCMATCHTHISDAY');
		$noarticle= $language->_('JLCNOMATCHES');
		$outstring = '';
		$todaystring = '';
		...
		$totalgamesstring .=  ' ';
		$totalgamesstring .= $language->_('JLCVALUEMATCHESMONTH') . ' ' .$this->monthNames[$month - 1] . ' ' . $year;
		...
		for ($x=0;$x<count($matches);$x++) {
			...
			if (!$nd || $nd->toFormat('%Y-%m-%d') != $da->toFormat('%Y-%m-%d')) {
				...
				$titletext .= ' ';
				$titletext .= ($today == $da->toFormat('%Y-%m-%d')) ? $language->_('JLCTODAY'): $language->_('JLCAT');
