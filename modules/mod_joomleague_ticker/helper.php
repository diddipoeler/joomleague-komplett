...
class modJoomleagueTickerHelper
{
	...
	function getMatches($numberofmatches, $projectid, $teamid, $selectiondate, $ordering = 'DESC', $round=0, $matchstatus, $bUseFavteams)
	{
		...
		$query_SELECT = ' SELECT matches.*, r.roundcode as roundcode, r.name as roundname, "dummy", '
	...
	function getCorrectDateFormat($format, $matchinfo, $offset=0)
	{
		...
		$time = array();
		foreach ($matchinfo as $getdata)
		{
			$unix_date[$id] = strtotime($getdata->match_date);
			// is it future date or past date
			if($now > ($unix_date[$id] + $first_time + $second_time + $pause)) {
				$time1[$id] = JHTML::_('date', $getdata->match_date, $format);
				$time[$id] = $time1[$id];

			} elseif($now < ($unix_date[$id])) {
				$time[$id] = JHTML::_('date', $getdata->match_date, $format);
			}
	...
	function getSelectionDate($daysback, $offset=0)
	{
