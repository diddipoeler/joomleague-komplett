...
class JoomleagueModelEditMatch extends JoomleagueModelProject
{
	...
	function isAllowed()
	{
		...
		if ($user->id != 0)
		{
			...
			if(($user->authorize('com_joomleague','result_match_edit')) ||
	...
	function setAllowed()
	{
		...
		$level = $results->config['minimum_permission'];
		$user_level = array("super administrator","administrator","manager","publisher","editor","author","registered");
		for ($i=0;$i<=$level;$i++){
			$auth->addACL('com_joomleague','result_match_edit','users',$user_level[$i]);
		}
