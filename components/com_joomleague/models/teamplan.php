...
class JoomleagueModelTeamPlan extends JoomleagueModelProject
{
	...
	function getTeamsRefPlan($config)
	{
		$games=$this->getMatchesRefering($config);
		if (!count($games))
		{
			return array();
		}
		...
		$query='	SELECT	tl.id AS projectteamid,
							...
							IF((ISNULL(tl.picture) OR (tl.picture="")), 
								(IF((ISNULL(t.picture) OR (t.picture="")), c.logo_small , t.picture)) , tl.picture) as picture,
							tl.project_id,
							t.id,
							t.name,
							t.short_name,
					...
					FROM #__joomleague_project_team tl
						...
						LEFT JOIN #__joomleague_project p ON p.id=tl.project_id
