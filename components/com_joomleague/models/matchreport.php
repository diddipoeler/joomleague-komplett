...
class JoomleagueModelMatchReport extends JoomleagueModelProject
{
	...
	function &getMatch()
	{
		...
		{
			$query=' SELECT m.*,DATE_FORMAT(m.time_present,"%H:%i") time_present,r.project_id, t1.team_id as team1_id, t2.team_id as team2_id '
			      .' FROM #__joomleague_match AS m '
			      .' INNER JOIN #__joomleague_round AS r on r.id = m.round_id '
                  .' LEFT JOIN #__joomleague_project_team AS t1 on t1.id = m.projectteam1_id '
                  .' LEFT JOIN #__joomleague_project_team AS t2 on t2.id = m.projectteam2_id '
	...
	function getSchemaHome($params)
  {
  $db = &Jfactory::getDBO();
  $bildpositionen = array();
  $position = 1;
  
  $schemahome = $params->get('formation1');
  //echo 'schemahome -> '.$schemahome.'<br>';
...
// 		echo '<pre><br>';
		...
		$paramsdata=$res;
    $paramsdefs=JPATH_COMPONENT_SITE.DS.'extensions'.DS.'jlextzetamatchreport'.DS.'admin'.DS.'assets'.DS.'extended'.DS.'rosterposition.xml';
    $extended = new JLGExtraParams($paramsdata,$paramsdefs);
    
    
    for($a=0; $a < 11; $a++)
    {
    $bildpositionen[$schemahome][$a][heim][oben] = $extended->get('JL_EXT_ROSTERPOSITIONS_'.$position.'_TOP');
    $bildpositionen[$schemahome][$a][heim][links] = $extended->get('JL_EXT_ROSTERPOSITIONS_'.$position.'_LEFT');
    $position++;
    }
...
// 		echo '<pre><br>';
  ...
  function getSchemaAway($params)
  {
  $db = &Jfactory::getDBO();
  $bildpositionen = array();
  $position = 1;
  
  $schemaaway = $params->get('formation2');
  //echo 'schemaaway -> '.$schemaaway.'<br>';
...
// 		echo '<pre><br>';
    ...
    for($a=0; $a < 11; $a++)
    {
    $bildpositionen[$schemaaway][$a][gast][oben] = $extended->get('JL_EXT_ROSTERPOSITIONS_'.$position.'_TOP');
    $bildpositionen[$schemaaway][$a][gast][links] = $extended->get('JL_EXT_ROSTERPOSITIONS_'.$position.'_LEFT');
    $position++;
    }
