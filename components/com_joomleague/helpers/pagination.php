...
class JoomleaguePagination
{
	...
	function pagenav($project)
	{
		...
		if ($firstRound['id'] != $roundid)
		{
			$params['r'] = $backward;
			$query = JURI::buildQuery($params);
			$link = JRoute::_('index.php?' . $query . '#jl_top');
			$prevlink = JHTML::link($link,JText::_('JL_GLOBAL_PREV'));

			$params['r'] = $firstRound['id'];
			$query = JURI::buildQuery($params);
			$link = JRoute::_('index.php?' . $query . '#jl_top');
			$firstlink = JHTML::link($link,JText::_('JL_GLOBAL_PAGINATION_START')) . $spacer4;
		}
		else
		{
			$prevlink = JText::_('JL_GLOBAL_PREV');
			$firstlink = JText::_('JL_GLOBAL_PAGINATION_START') . $spacer4;
		}
		if ($lastRound['id'] != $roundid)
		{
			...
			$link = JRoute::_('index.php?'.$query.'#jl_top');
			$nextlink = $spacer4;
			$nextlink .= JHTML::link($link,JText::_('JL_GLOBAL_NEXT'));

			$params['r'] = $lastRound['id'];
			$query = JURI::buildQuery($params);
			$link = JRoute::_('index.php?' . $query . '#jl_top');
			$lastlink = $spacer4 . JHTML::link($link,JText::_('JL_GLOBAL_PAGINATION_END'));
		}
		else
		{
			$nextlink = $spacer4 . JText::_('JL_GLOBAL_NEXT');
			$lastlink = $spacer4 . JText::_('JL_GLOBAL_PAGINATION_END');
		}
