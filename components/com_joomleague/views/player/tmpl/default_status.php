		...
		( isset($this->teamPlayer->away) && $this->teamPlayer->away > 0 ) )
{
	...
	<table class="status">
		...
		if ($this->teamPlayer->injury > 0)
		{
			...
			$injury_date = JHTML::date($this->teamPlayer->injury_date, JText::_('JL_GLOBAL_MATCHDAYDATE'));
			if(isset($this->teamPlayer->rinjury_from))
			$injury_date .= " - ".$this->teamPlayer->rinjury_from;

			//injury end
			$injury_end = JHTML::date($this->teamPlayer->injury_end, JText::_('JL_GLOBAL_MATCHDAYDATE'));
		...
		if ($this->teamPlayer->suspension > 0)
		{
			...
			//suspension start
			$suspension_date = JHTML::date($this->teamPlayer->suspension_date, JText::_('JL_GLOBAL_MATCHDAYDATE'));
			if(isset($this->teamPlayer->rsusp_from))
			$suspension_date .= " - ".$this->teamPlayer->rsusp_from;
			
			$suspension_end = JHTML::date($this->teamPlayer->suspension_end	, JText::_('JL_GLOBAL_MATCHDAYDATE'));
		...
		if ($this->teamPlayer->away > 0)
		{
			...
			//suspension start
			$away_date = JHTML::date($this->teamPlayer->away_date, JText::_('JL_GLOBAL_MATCHDAYDATE'));
			if(isset($this->teamPlayer->raway_from))
			$away_date .= " - ".$this->teamPlayer->raway_from;

			$away_end = JHTML::date($this->teamPlayer->away_end, JText::_('JL_GLOBAL_MATCHDAYDATE'));
