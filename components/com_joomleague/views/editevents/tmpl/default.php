...
<!--
	// url for ajax
	var baseajaxurl='<?php echo JURI::base().'index.php?option=com_joomleague&controller=editevents&format=raw'; ?>';
...
<div id="editevents">
	<h2 style='text-align:center;'>
		<?php
		echo JText::_('JL_EDIT_EVENTS_TITLE');
		echo '<br />';
		echo JText::sprintf('JL_EDIT_EVENTS_MATCH_TEAMS',$this->hometeam->name,$this->awayteam->name);
		?>
	</h2>
	<form name="startingSquadsForm" id="startingSquadsForm" method="post">
		<?php
		$paneStartOffset=9;
		$pane =& JPane::getInstance('Tabs',array('startOffset'=>$paneStartOffset),false);
		...
		echo $pane->startPane('pane');
			...
			echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_HOME_ROSTER'),'panel_0');
				echo $pane->startPane('pane1');

					echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_EDIT_HOME_SUBST'),'panel_1');
						echo $this->loadTemplate('edit_home_substitution');
					echo $pane->endPanel();

					echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_EDIT_HOME_PLAYER'),'panel_2');
						echo $this->loadTemplate('edit_home_player');
					echo $pane->endPanel();

					echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_EDIT_HOME_STAFF'),'panel_3');
			...
			echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_AWAY_ROSTER'),'panel_4');
				echo $pane->startPane('pane2');

					echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_EDIT_AWAY_SUBST'),'panel_5');
						echo $this->loadTemplate('edit_away_substitution');
					echo $pane->endPanel();

					echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_EDIT_AWAY_PLAYER'),'panel_6');
						echo $this->loadTemplate('edit_away_player');
					echo $pane->endPanel();

					echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_EDIT_AWAY_STAFF'),'panel_7');
			...
			echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_EDIT_REFEREES'),'panel_8');
				echo $this->loadTemplate('edit_referees');
			echo $pane->endPanel();

			echo $pane->startPanel(JText::_('JL_EDIT_EVENTS_EDIT_EVENTS'),'panel_9');
		...
		<input type="hidden" name="controller" value="editevents" />
