...
<fieldset class="adminform">
	<legend>
		<?php
		echo JText::sprintf(	'JL_EDITMATCH_SCOREDETAILS_TITLE',
								'<i>' . $this->team1->name . '</i>',
								'-',
								'<i>' . $this->team2->name . '</i>' );
		?>
		 ...
		 ( !isset( $this->overallconfig['edit_results_below'] ) || $this->overallconfig['edit_results_below'] == 0 ) )
	{
		...
		{
			$xrounds = array();
			$xrounds[] = JHTML::_( 'select.option', 0, JText::_('JL_EDITMATCH_SCOREDETAILS_REGULAR_TIME'), 'value', 'text' );
			$xrounds[] = JHTML::_( 'select.option', 1, JText::_('JL_EDITMATCH_SCOREDETAILS_OVER_TIME'), 'value', 'text' );
			$xrounds[] = JHTML::_( 'select.option', 2, JText::_('JL_EDITMATCH_SCOREDETAILS_SHOOTOUTS'), 'value', 'text' );
	...
	<table border="0">
			   ...
			   $this->overallconfig['edit_results_below'] == 0 ) )
		{
			...
			<tr>
				<td align="left">
					<?php
					echo JText::_( 'JL_EDITMATCH_SCOREDETAILS_PART_RESULT' );
					?>
				</td>
				<td align="right">
					<?php
			...
			if ( $this->project->use_legs == 1 )
			{
				...
				<tr>
					<td>
						...
						{
							echo JText::_('JL_EDITMATCH_SCOREDETAILS_LEGS');
						}
						else
						{
		...
		<tr>
			<td>
				<?php
				echo JText::_( 'JL_EDITMATCH_SCOREDETAILS_BONUS_POINTS' );
				?>
			</td>
			<td align="right">
		...
		<tr>
			<td>
				<?php
				echo JText::_( 'JL_EDITMATCH_SCOREDETAILS_SCORE_NOTICE' );
				?>
			</td>
			<td colspan="3">
			 ...
			 ( !isset($this->overallconfig['edit_results_below'] ) || $this->overallconfig['edit_results_below'] == 0 ) )
		{
			if( $this->project->allow_add_time )
			{
				...
				<tr>
					<td>
						<?php
						echo JText::_('JL_EDITMATCH_SCOREDETAILS_RESULT_TYPE');
						?>
					</td>
					<td>
						<?php
