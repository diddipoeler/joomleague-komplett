...
<table style='width:96%; border-width:0px; border-style:solid; border-spacing:0; text-align:center; '>
	...
	<tr class="sectiontableheader">
		<?php if ($this->config['show_small_logo'])		{ ?><th class="club_logo"><?php echo JText::_( 'JL_CLUBS_LOGO' ); ?></th><?php } ?>
		<?php if ($this->config['show_medium_logo'])	{ ?><th class="club_logo"><?php echo JText::_( 'JL_CLUBS_LOGO' ); ?></th><?php } ?>
		<?php if ($this->config['show_big_logo'])		{ ?><th class="club_logo"><?php echo JText::_( 'JL_CLUBS_LOGO' ); ?></th><?php } ?>
		<th class="club_name"><?php echo JText::_( 'JL_CLUBS_CLUBNAME' ); ?></th>
		<?php if ($this->config['show_club_teams'])		{ ?><th class="club_teams"><?php echo JText::_( 'JL_CLUBS_TEAMS' ); ?></th><?php } ?>
		<?php if ($this->config['show_address'])		{ ?><th class="club_address"><?php echo JText::_( 'JL_CLUBS_ADDRESS' ); ?></th><?php } ?>
	</tr>
	</thead>
	<?php
	$k = 0;
	foreach ($this->clubs as $club)
	{
		...
		$title			= JText::sprintf( 'JL_CLUBS_TITLE2', $club->name );
				...
				<td>
					<?php
						foreach ($club->teams as $team)
						{
							...
							if ( ( is_null( $team->picture) ) || ( !file_exists( $picture ) ) )
							{
								/*
								$picture = 'media/com_joomleague/placeholders/placeholder_small.gif';
								echo JHTML::image($picture,
												  $team->name,
												  array( "title" => $team->name )
												 );
								*/
							}
							else
							{
								$height = $this->config['team_picture_small_height'];
								echo JHTML::image($picture,
												  $team->name,
												  array( "title" => $team->name, "height" => $height )
												 );
								echo '<br />';
							}
