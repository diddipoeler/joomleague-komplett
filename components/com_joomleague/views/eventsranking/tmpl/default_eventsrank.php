...
<table class="eventsranking">
	...
	<tr class="sectiontableheader">
		<th class="rank"><?php echo JText::_('JL_EVENTSRANKING_RANK'); ?></th>

		<?php if ($this->config['show_picture_thumb'] == 1): ?>
		<th class="td_c">&nbsp;</th>
		<?php endif; ?>

		<th class="td_l"><?php echo JText::_('JL_EVENTSRANKING_PLAYER_NAME'); ?></th>

		<?php if($this->config['show_nation'] == 1): ?>
		<th class="td_c">&nbsp;</th>
		<?php endif; ?>

		<th class="td_l"><?php echo JText::_('JL_EVENTSRANKING_TEAM'); ?></th>
	...
	<tr class="<?php echo $class; ?>"<?php echo $favStyle; ?>>
		...
		<td class="td_l playerteam" width="30%">
			...
			if ($isFavTeam) {
				...
				$this->config['names'] = $row->tname;
				$teamName = JoomleagueHelper::formatTeamName($team,"t".$row->tid,$this->config, $highlightFavTeam,0,$this->project->id);
			} else {
			  $teamName = $row->tname;
			}
...
<?php echo JHTML::link($link=JoomleagueHelperRoute::getEventsRankingRoute($this->project->id, (isset($this->division->id) ? $this->division->id : 0), $this->teamid, $rows->id, (isset($this->matchid) ? $this->matchid : 0)), JText::_('JL_EVENTSRANKING_MORE')); ?>
</div>
<?php else: ?>
