	...
	<div id='editcell'>
		...
		<table width='100%' class='adminlist'>
			...
			for ($i=0, $n=count($this->items); $i < $n; $i++)
			{
				...
				$link			= JRoute::_('index.php?option=com_joomleague&controller=predictiongame&task=edit&cid[]=' . $row->id);
				?>
			...
			if ( $this->dPredictionID > 0 )
			{
				...
				if ( count( $this->predictionProjects ) > 0 )
				{
					foreach ( $this->predictionProjects AS $pred_project )
					{
						...
						$link = JRoute::_(	'index.php?option=com_joomleague&view=predictiongame' .
											'&controller=predictiongame' .
											'&task=predsettings&cid[]=' . $pred_project['id'] );
						?>
						<tr class='<?php echo "row$k"; ?>'>
