...
// 		$this->_params = $this->_project->getTemplateConfig('ranking');
    $this->_params = $this->_project->getTemplateConfig('rankingalltime');
...
//     echo '</pre><br>';
    ...
    $mode     	= $this->_mode;
  	...
  	$project->points_after_regular_time = JRequest::getVar( "points", 0 );
  	
    if ( !$project->points_after_regular_time )
  	{
    // retrieve the value of the state variable. If no value is specified,
    // the specified default value will be returned.
    // function syntax is getUserState( $key, $default );
    $project->points_after_regular_time = $mainframe->getUserState( "com_joomleague.alltimetablepoints", '3,1,0' );
    }
    else
    {
    // store the variable that we would like to keep for next time
    // function syntax is setUserState( $key, $value );
    $mainframe->setUserState( "com_joomleague.alltimetablepoints", $project->points_after_regular_time );
    }
			...
			|| !isset( $data->_teams[$match->projectteam2_id]) || $data->_teams[$match->projectteam2_id]->_is_in_score === 0  )
		{
			...
			{
				$home = new JLGRankingalltimeTeam(0); //in that case, $data wont be affected
			}
			if ($mode == 0 || $mode == 2)
			{
			...
			{
				$away = new JLGRankingalltimeTeam(0); //in that case, $data wont be affected
			}
	...
	function _initTeams($pid)
	{
		...
		{
			$t = new JLGRankingalltimeTeam($r->ptid);
...
// echo '_getMatches this->project_ids -> '.$this->project_ids.'<br>';
// echo '_getMatches this->project_ids<br><pre>';
// print_r($this->project_ids);
// echo '</pre>';
...
// echo '_getMatches projekt -> '.$pid.'<br>';
// echo '_getMatches matches<br><pre>';
// print_r($matches);
// echo '</pre>';
	 ...
	 *
	 * @param object match
	 * @param int from: first round
	 * @param int to: last round
	 * @param int division id
