...
?>
	...
	function chooseCSSFiles($template, $t_dir, $t_files, $option, & $client, $editor)
	{
		//JRequest::setVar( 'hidemainmenu', 1 );
?>
		<form action="index.php" method="post" name="adminForm">
...
?>
		...
		<input type="hidden" name="task" value="" />
		<input type="hidden" name="controller" value="jlextcsseditor" />
		<input type="hidden" name="boxchecked" value="0" />
		<input type="hidden" name="client" value="<?php echo $client->id;?>" />
		</form>
		<?php
	...
	* @param string The option
	*/
	function editCSSSource($template, $filename, & $content, $option, & $client, & $ftp, $editor)
	{
		//JRequest::setVar( 'hidemainmenu', 1 );
    ...
    switch($editor)
			{
		      ...
		      $css_path = JPATH_SITE.DS.'components'.DS.$option.DS.'extensions'.DS.'jlextcsseditor'.DS.'assets'.DS.'css'.DS.$filename;
		      //$files = JFolder::files($dir, '\.css$', false, false);
					break;

				case 'XML':
				  $css_path = JPATH_SITE.DS.'administrator'.DS.'components'.DS.$option.DS.'assets'.DS.'extended'.DS.$filename;
