	...
	function cpCopyTeams($post,$source_to_copy_division)
	{
		...
		if ($results = $this->_db->loadAssocList())
		{
			foreach($results as $result)
			{
				...
				{
					echo JText::sprintf('JL_ADMIN_PROJECTTEAM_MODEL_TP_COPIED',$from_projectteam_id).'<br />';
				}
				else
				{
					echo JText::sprintf('JL_ADMIN_PROJECTTEAM_MODEL_ERROR_TP_COPIED',$from_projectteam_id).'<br />'.$model->getError().'<br />';
				}

				//copy project team-staff
				if ($mdlTeamStaff->cpCopyTeamStaffs($from_projectteam_id,$to_projectteam_id))
				{
					echo JText::sprintf('JL_ADMIN_PROJECTTEAM_MODEL_TS_COPIED',$from_projectteam_id).'<br />';
				}
				else
				{
					echo JText::sprintf('JL_ADMIN_PROJECTTEAM_MODEL_ERROR_TS_COPIED',$from_projectteam_id).'<br />'.$model->getError().'<br />';
				}
				...
				if ($results = $this->_db->loadAssocList())
				{
					foreach($results as $result)
					{
						...
						{
							echo JText::sprintf('JL_ADMIN_PROJECTTEAM_MODEL_ERROR_TRAINING_COPIED',$from_projectteam_id).'<br />';
						}
					}
				}
			}
