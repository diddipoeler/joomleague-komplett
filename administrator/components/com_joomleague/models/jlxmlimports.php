...
//die();
	...
	$databaseID = $mainframe->getUserState($option.'databaseID');
	...
	private function _getClubName($club_id)
	{
		...
		return '#Error in _getClubName club_id -> ('.$club_id.') #';
	}
	...
	private function _getTeamName($team_id)
	{
		...
		return '#Error in _getTeamName team_id -> ('.$team_id.') #';
	}
...
if ( $this->debug_info )
{
echo $this->pane->startPanel('function _importPlayground','function _importPlayground');      
...
if ( $this->debug_info )
{
echo $this->pane->startPanel('function _importClubs','function _importClubs');    
...
$this->dump_variable("this->_newclubs", $this->_newclubs);
				...
				if ( $object = $this->_db->loadObject() )
				{
					...
					$my_text .= '<br />';
                    // diddipoeler
                    // update clubdata standard_playground
                    if ( $p_club->standard_playground )
                    {
                    $p_club_update =& $this->getTable('club');
                    $p_club_update->set('id',$object->id);
                    $p_club_update->standard_playground = $p_club->standard_playground;
                    if ($p_club_update->store() === false)
					{
					    $my_text .= '<span style="color:'.$this->storeFailedColor.'"><strong>';
						$my_text .= JText::_('2.a) Error in function update importClubs').'</strong></span><br />';
						$my_text .= JText::sprintf('Clubname: %1$s',$p_club->name).'<br />';
						$my_text .= JText::sprintf('Error-Text #%1$s#',$this->_db->getErrorMsg()).'<br />';
						$my_text .= '<pre>'.print_r($p_club_update,true).'</pre>';
						$this->_success_text['Importing general club data:']=$my_text;
						return false;   
					}
                    else
                    {
                    $my_text .= '<span style="color:'.$this->updateSuccessColor.'">';
					$my_text .= JText::sprintf('2.b) Playground Update existing club data: %1$s',"</span><strong>$object->name</strong>");
					$my_text .= '<br />';    
                    }   
...
if ( $this->debug_info )
{
echo $this->pane->startPanel('function _convertNewPlaygroundIDs','function _convertNewPlaygroundIDs');    
...
$this->dump_variable("this->_convertPlaygroundID new_pg_id", $new_pg_id);
$this->dump_variable("p_playground", $p_playground);
}
...
$this->dump_variable("this->_convertClubID new_club_id", $new_club_id);				    
					...
					if ( isset($p_playground->club_id) && ( $p_playground->club_id == $key ) && $new_club_id )
					{
...
//$this->dump_variable("import_team", $import_team);
				...
				/*
                // falsche abfrage, legt zusätzliches team an.
                $query="SELECT	id,
								name,
								short_name,
								middle_name,
...
//$this->dump_variable("_convertTeamID", $this->_convertTeamID);
//$this->dump_variable("_convertPlaygroundID", $this->_convertPlaygroundID);
...
echo $this->pane->startPane('pane');    
       ...
       $this->_datas=$this->getData(); 
		...
		if (is_array($post) && count($post) > 0)
		{
          ...
          if ( $this->debug_info )
          {
          echo $this->pane->startPanel('function importData','function importData');     
