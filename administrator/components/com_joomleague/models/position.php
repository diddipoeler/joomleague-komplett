...
class JoomleagueModelPosition extends JoomleagueModelItem
{
	...
	function delete($cid=array())
	{
		...
		if (count($cid))
		{
			...
			{
				$this->setError(JText::_('JL_ADMIN_POSITION_MODEL_ERROR_P_POSITION_EXISTS'));
				return false;
			}
			...
			if ($this->_db->loadResult())
			{
				$this->setError(JText::_('JL_ADMIN_POSITION_MODEL_ERROR_P_REFEREE_EXISTS'));
				return false;
			}
			...
			if ($this->_db->loadResult())
			{
				$this->setError(JText::_('JL_ADMIN_POSITION_MODEL_ERROR_PLAYER_EXISTS'));
				return false;
			}
			...
			if ($this->_db->loadResult())
			{
				$this->setError(JText::_('JL_ADMIN_POSITION_MODEL_ERROR_STAFF_EXISTS'));
				return false;
			}
			...
			if ($this->_db->loadResult())
			{
				$this->setError(JText::_('JL_ADMIN_POSITION_MODEL_ERROR_PERSON_EXISTS'));
				return false;
			}

			$query="DELETE FROM #__joomleague_position WHERE id IN ($cids)";
			$this->_db->setQuery($query);
			if (!$this->_db->query())
			{
				$this->setError($this->_db->getErrorMsg());
