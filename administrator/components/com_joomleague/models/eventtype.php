...
class JoomleagueModelEventtype extends JoomleagueModelItem
{
	...
	function delete($cid=array())
	{
		...
		if (count($cid))
		{
			...
			{
				$this->setError(JText::_('JL_ADMIN_EVENT_MODEL_ERROR_MATCHES_EXISTS'));
				return false;
			}
			...
			if ($this->_db->getAffectedRows())
			{
				$this->setError(JText::_('JL_ADMIN_EVENT_MODEL_ERROR_POSITION_EXISTS'));
				return false;
			}
			$query="DELETE FROM #__joomleague_eventtype WHERE id IN ($cids)";
			$this->_db->setQuery($query);
			if(!$this->_db->query())
			{
				$this->setError($this->_db->getErrorMsg());
	...
	/**
	* Method to return the query that will obtain all ordering versus eventtypes
	...
	* @since 1.5
	*/
	function getOrderingAndEventtypeQuery()
	{
		return 'SELECT ordering AS value, name AS text FROM #__joomleague_eventtype ORDER BY ordering';
	}
