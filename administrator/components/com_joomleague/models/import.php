	...
	function import($fieldsname, $data, $replace = true, $table)
	{
		...
		foreach ($data AS $row)
		{
			...
			if (!$object->check())
			{
				...
				$rec['errormsg'] = JText::_('JL_GLOBAL_ERROR_CHECK').$object->getError();
				continue ;
			}
			...
			if ($replace)
			{
				...
				if (!$object->insertIgnore())
				{
					if (!$object->store())
					{
						//echo JText::_('JL_GLOBAL_ERROR_STORE').$this->_db->getErrorMsg()."\n";
						$rec['exists']++;
						continue ;
					}
					else
			...
			else
			{
				if (!$object->store())
				{
					...
					//$rec['errormsg'] =  JText::_('JL_GLOBAL_ERROR_STORE').$this->_db->getErrorMsg()."<br \>\n";
					$rec['exists']++;
					continue ;
				}
				else
