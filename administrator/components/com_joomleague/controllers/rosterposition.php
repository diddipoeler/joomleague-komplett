 ...
 * @since	0.1
 */
class JoomleagueControllerrosterposition extends JoomleagueCommonController
{
	...
	function save()
	{
		...
		if ($model->store($post))
		{
			$msg=JText::_('JL_ADMIN_ROSTERPOSITIONS_CTRL_SAVED');
		}
		else
		{
			$msg=JText::_('JL_ADMIN_ROSTERPOSITIONS_CTRL_ERROR_SAVE').$model->getError();
		}
		...
		{
			$link='index.php?option=com_joomleague&controller=rosterposition&task=edit&cid[]='.$post['id'];
		}
	...
	function remove()
	{
		...
		{
			$msg=JText::_('JL_ADMIN_ROSTERPOSITIONS_CTRL_DELETED');
		}
