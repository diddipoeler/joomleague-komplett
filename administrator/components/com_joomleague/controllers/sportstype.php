 ...
 * @since	0.1
 */
class JoomleagueControllerSportsType extends JoomleagueCommonController
{
	...
	function save()
	{
		...
		if ($model->store($post))
		{
			$msg=JText::_('JL_ADMIN_SPORTTYPE_CTRL_STAT_SAVED');
		}
		else
		{
			$msg=JText::_('JL_ADMIN_SPORTTYPE_CTRL_ERROR_SAVE').$model->getError();
		}
		// Check the table in so it can be edited.... we are done with it anyway
		$model->checkin();
		if ($this->getTask()=='save')
		{
			$link='index.php?option=com_joomleague&view=sportstypes';
		}
		else
		{
			$link='index.php?option=com_joomleague&controller=sportstype&task=edit&cid[]='.$post['id'];
		}
	...
	function remove()
	{
		...
		{
			$msg='JL_ADMIN_SPORTTYPE_CTRL_DELETED';
		}
	...
	function saveorder()
	{
		...
		$this->setRedirect('index.php?option=com_joomleague&view=sportstypes',$msg);
	}
