 ...
 * @since	0.1
 */
class JoomleagueControllerProjectteam extends JoomleagueCommonController
{
	...
	function edit()
	{
		...
		if ($model->isCheckedOut($user->get('id'))) {
			$this->setRedirect('index.php?option=com_joomleague&controller=projectteam&view=projectteams', JText::_('EDITED BY ANOTHER ADMIN'));
		}
...
//  echo 'storechangeteams<pre>'.print_r($post,true).'</pre>';
  ...
  $this->setRedirect('index.php?option=com_joomleague&controller=projectteam&view=projectteams');  
  //parent::display();
  }
  ...
  function editlist()
	...
	function save_teamslist()
	{
		...
		if ($model->store($post))
		{
			...
			$cache->clean();
			$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_SAVED');
		}
		else
		{
			$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_ERROR_SAVE') . $model->getError();
		}

		// Check the table in so it can be edited.... we are done with it anyway
		//$model->checkin();
		$link = 'index.php?option=com_joomleague&controller=projectteam&view=projectteams';
	...
	function save()
	{
		...
		if (isset($post['add_trainingData']))
		{
			...
			{
				$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_TRAINING');
			}
			else
			{
				$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_ERROR_TRAINING').$model->getError();
			}
			//echo $msg;
		}
		...
		if (isset($post['tdCount'])) // Existing Team Trainingdata
		{
			...
			{
				$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_TRAINING_SAVED');
			}
			else
			{
				$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_TRAINING_ERROR_SAVE').$model->getError();
			}

			if ($model->checkAndDeleteTrainigData($post))
			{
				$msg .= ' - '.JText::_('JL_ADMIN_P_TEAM_CTRL_TRAINING_DELETED');
			}
			else
			{
				$msg = ' - '.JText::_('JL_ADMIN_P_TEAM_CTRL_TRAINING_ERROR_DELETED').$model->getError();
			}
			$msg .= ' - ';
		}
		...
		if ($model->store($post))
		{
			...
			$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_TEAM_SAVED');
		}
		else
		{
			$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_TEAM_ERROR_SAVE').$model->getError();
		}
		...
		{
			$link = 'index.php?option=com_joomleague&controller=projectteam&task=edit&cid[]=' . $post['id'];
		}
		//echo $msg;
		$this->setRedirect($link,$msg);
	}
	...
	function saveshort()
	{
		...
		if ($model->storeshort($cid, $post))
		{
			...
			$cache = & JFactory::getCache('joomleague.project'.$project_id);
			$cache->clean();
			$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_UPDATED');
		}
		else
		{
			$msg = JText::_('JL_ADMIN_P_TEAM_CTRL_ERROR_UPDATED') . $model->getError();
		}
	...
	function orderup()
	{
		...
		$this->setRedirect('index.php?option=com_joomleague&view=projectteam');
	}
	...
	function saveorder()
	{
		...
		$this->setRedirect('index.php?option=com_joomleague&view=projectteam', $msg);
	}
	...
	function copy()
	{
		...
		{
			$msg = JText::_('JL_ADMIN_PROJECTTEAMS_COPY_SUCCESS');	
		}
		$this->setRedirect('index.php?option=com_joomleague&view=projectteams&controller=projectteam', $msg, $type);
		$this->redirect();
	}
}
?>