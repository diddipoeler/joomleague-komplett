 ...
 * @since		1.5
 */
class JoomleagueControllerImport extends JoomleagueCommonController
{
	/**
	 * Constructor
	 *
	...
	function dispatch()
	{
		switch($this->getTask())
		{
			...
			default:
				$msg = JText :: _('JL_ADMIN_IMPORT_CTRL_NOT_EXIST');
	...
	function import($table,$view)
	{
		...
		if ($csvimport)
		{
			...
			if (!$handle)
			{
				$msg = JText :: _('JL_ADMIN_IMPORT_CTRL_CANNOT_OPEN');
			...
			if (!count($fields))
			{
				$msg = JText :: _('JL_ADMIN_IMPORT_CTRL_ERROR_PARSING');
				$this->setRedirect('index.php?option=com_joomleague&view='.$view,$msg,'error');
				return;
			}
			else
			...
			while (($data = fgetcsv($handle,10000,$delimiter,'"')) !== FALSE)
			{
				...
				if ($numfields != $num)
				{
					$msg[] = JText :: _('JL_ADMIN_IMPORT_CTRL_WRONG_NUMBER_OF_FIELDS');
				}
				else
				{
					$r = array ();
			...
			$msg[] = JText :: _('JL_ADMIN_IMPORT_CTRL_TOTAL_RECORDS_FOUND') . count($records);
			...
			if (count($records))
			{
				...
				$msg[]= $result['errormsg'];
				$msg[] = JText :: _('JL_ADMIN_IMPORT_CTRL_TOTAL_ADDED_RECORDS') . $result['added'];
				$msg[] = JText :: _('JL_ADMIN_IMPORT_CTRL_TOTAL_UPDATED_RECORDS'). $result['updated'];
				$msg[] = JText :: _('JL_ADMIN_IMPORT_CTRL_TOTAL_EXISTS_RECORDS'). $result['exists'];
			}
