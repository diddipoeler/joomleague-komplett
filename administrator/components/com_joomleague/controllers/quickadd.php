 ...
 * @since 1.5
 */
class JoomleagueControllerQuickAdd extends JoomleagueCommonController
{
	...
	function addPlayer()
	{
		...
		if ($res) {
			$msgtype = 'message';
			$msg = JText::_('JL_ADMIN_QUICKADD_CTRL_PERSON_ASSIGNED');
		}
		else {
			$msgtype = 'error';
			$msg = $model->getError();
		...
		$this->setRedirect("index.php?option=com_joomleague&view=teamplayers&controller=teamplayer&project_team_id=".$projectteam_id, $msg);
	}
	...
	function addStaff()
	{
		...
		if (!$personid)
		{
			...
			$model->store($data);
			$personid = $model->_db->insertid();
		}

		if (!$personid) {
			$msg = Jtext::_('JL_ADMIN_QUICKADD_CTRL_PERSON_ASSIGNED');
			$this->setRedirect("index.php?option=com_joomleague&view=teamstaffs&controller=teamstaff&project_team_id=".$projectteam_id, $msg, 'error');
		}
		...
		$msg = Jtext::_('JL_ADMIN_QUICKADD_CTRL_PERSON_ASSIGNED');
		$this->setRedirect("index.php?option=com_joomleague&view=teamstaffs&controller=teamstaff&project_team_id=".$projectteam_id, $msg);
	}
	...
	function addReferee()
	{
		...
		if (!$personid) {
			$msg = Jtext::_('JL_ADMIN_QUICKADD_CTRL_PERSON_ASSIGNED');
			$this->setRedirect("index.php?option=com_joomleague&view=projectreferees&controller=projectreferee&projectid=".$project_id, $msg, 'error');
		}
		...
		$msg = Jtext::_('JL_ADMIN_QUICKADD_CTRL_PERSON_ASSIGNED');
		$this->setRedirect("index.php?option=com_joomleague&view=projectreferees&controller=projectreferee&projectid=".$project_id, $msg);
	}
	...
	function addTeam()
	{
		...
		if (!$teamid)
		{
			...
			$model->store($data);
			$teamid = $model->_db->insertid();
		}

		if (!$teamid) {
			$msg = Jtext::_('JL_ADMIN_QUICKADD_CTRL_ERROR_TEAM');
			$this->setRedirect("index.php?option=com_joomleague&view=projectteams&controller=projectteam&projectid=".$project_id, $msg, 'error');
		}
		...
		$msg = Jtext::_('JL_ADMIN_QUICKADD_CTRL_TEAM_ASSIGNED');
		$this->setRedirect("index.php?option=com_joomleague&view=projectteams&controller=projectteam&projectid=".$project_id, $msg);
	}
}
?>