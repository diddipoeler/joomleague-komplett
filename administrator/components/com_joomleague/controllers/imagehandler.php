 ...
 * @since 0.9
 */
class JoomLeagueControllerImagehandler extends JoomleagueCommonController
{
	/**
	 * Constructor
	 *
	...
	function upload()
	{
		...
		if ( empty( $file['name'] ) )
		{
			echo "<script> alert('".JText::_( 'JL_ADMIN_IMAGEHANDLER_CTRL_IMAGE_EMPTY' )."'); window.history.go(-1); </script>\n";
			$mainframe->close();
		}
		...
		{
			echo "<script> alert('".JText::_( 'JL_ADMIN_IMAGEHANDLER_CTRL_UPLOAD_FAILED' )."'); window.history.go(-1); </script>\n";
			$mainframe->close();

		}
		else
		{
			echo "<script> alert('" . JText::_( 'JL_ADMIN_IMAGEHANDLER_CTRL_UPLOAD_COMPLETE' ) . "'); window.history.go(-1); window.parent.selectImage_".$type."('$filename', '$filename','$field'); </script>\n";
			$mainframe->close();
		}
	...
	function delete()
	{
		...
		if ( count( $images ) )
		{
			foreach ( $images as $image )
			{
				...
				{
					JError::raiseWarning( 100, JText::_( 'JL_ADMIN_IMAGEHANDLER_CTRL_UNABLE_TO_DELETE' ) . ' ' . htmlspecialchars( $image, ENT_COMPAT, 'UTF-8' ) );
					continue;
				}
