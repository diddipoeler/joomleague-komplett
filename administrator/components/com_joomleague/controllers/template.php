 ...
 * @since	0.1
 */
class JoomleagueControllerTemplate extends JoomleagueCommonController
{
	...
	function apply()
	{
		...
		if ($model->store($post))
		{
			$msg=JText::_('JL_ADMIN_TEMPLATE_CTRL_SAVED_TEMPLATE');
		}
		else
		{
			$msg=JText::sprintf('JL_ADMIN_TEMPLATE_CTRL_ERROR_SAVE_TEMPLATE',$index).' '.$model->getError();
		}
		// Check the table in so it can be edited.... we are done with it anyway
		$model->checkin();
		if ($this->getTask() == 'save')
		{
			$link='index.php?option=com_joomleague&controller=template&view=templates';
		}
		else
		{
			$link='index.php?option=com_joomleague&controller=template&task=edit&cid[]='.$post['select_id'];
		}
	...
	function save()
	{
		...
		else
		{
			for ($index=0; $index < count($cid); $index++)
			{
				...
				if ($model->store($post))
				{
					$msg=JText::_('JL_ADMIN_TEMPLATE_CTRL_REBUILD_TEMPLATES');
				}
				else
				{
					$msg=JText::sprintf('JL_ADMIN_TEMPLATE_CTRL_ERROR_REBUILD_TEMPLATE',$index).' '.$model->getError();
					break;
				}
				// Check the table in so it can be edited.... we are done with it anyway
				$model->checkin();
		...
		{
			$link='index.php?option=com_joomleague&controller=template&task=edit&cid[]='.$post['id'];
		}
		$this->setRedirect($link,$msg);
	}
	...
	function remove()
	{
		...
		foreach ($cid AS $id)
		{
			if ($isMaster[$id])
			{
				echo "<script> alert('" . JText::_('JL_ADMIN_TEMPLATE_CTRL_DELETE_WARNING') . "'); window.history.go(-1); </script>\n";
				return;
			}
		}
		$model=$this->getModel('template');
		...
		$this->setRedirect('index.php?option=com_joomleague&controller=template&view=templates');
	}
	...
	function cancel()
	{
		...
		$model=$this->getModel('templates');
		//$model->checkin();
		$this->setRedirect('index.php?option=com_joomleague&view=templates&controller=template');
	}
	...
	function masterimport()
	{
		...
		{
			$msg=JText::_('JL_ADMIN_TEMPLATE_CTRL_IMPORTED_TEMPLATE');
		}
		else
		{
			$msg=JText::_('JL_ADMIN_TEMPLATE_CTRL_ERROR_IMPORT_TEMPLATE').$model->getError();
		}
		$this->setRedirect('index.php?option=com_joomleague&controller=template&view=templates',$msg);
	}
