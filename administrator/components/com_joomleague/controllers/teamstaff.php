 ...
 * @since	0.1
 */
class JoomleagueControllerTeamSTaff extends JoomleagueCommonController
{
	...
	function save_teamstaffslist()
	{
		...
		if ($model->store($post))
		{
			$msg=JText::_('JL_ADMIN_TEAMSTAFF_CTRL_TEAMSTAFF_SAVED');
		}
		else
		{
			$msg=JText::_('JL_ADMIN_TEAMSTAFF_CTRL_ERROR_TEAMSTAFF_SAVED').$model->getError();
		}
		// Check the table in so it can be edited.... we are done with it anyway
		//$model->checkin();
		$link='index.php?option=com_joomleague&controller=teamstaff&view=teamstaffs';
	...
	function save()
	{
		...
		if ($model->store($post))
		{
			$msg=JText::_('JL_ADMIN_TEAMSTAFF_CTRL_SAVED');
		}
		else
		{
			$msg=JText::_('JL_ADMIN_TEAMSTAFF_CTRL_ERROR_SAVE').$model->getError();
		}
		// Check the table in so it can be edited.... we are done with it anyway
		$model->checkin();
		...
		{
			$link='index.php?option=com_joomleague&controller=teamstaff&task=edit&cid[]='.$post['id'];
		}
		#echo $msg;
		$this->setRedirect($link,$msg);
	}
	...
	function saveshort()
	{
		...
		{
			$msg=JText::_('JL_ADMIN_TEAMSTAFF_CTRL_UPDATED');
		}
		else
		{
			$msg=JText::_('JL_ADMIN_TEAMSTAFF_CTRL_ERROR_UPDATED').$model->getError();
		}
		$link='index.php?option=com_joomleague&controller=teamstaff&view=teamstaffs';
		$this->setRedirect($link,$msg);
	}
	...
	function publish()
	{
		...
		$this->setRedirect('index.php?option=com_joomleague&controller=teamstaff&view=teamstaffs');
	}
	...
	function saveorder()
	{
		...
		$this->setRedirect('index.php?option=com_joomleague&controller=teamstaff&view=teamstaffs',$msg);
	}
	...
	function assign()
	{
		...
		$msg=JText::_('JL_ADMIN_TEAMSTAFF_CTRL_ASSIGN');
		$this->setRedirect('index.php?option=com_joomleague&view=persons&layout=assignplayers&type=1&hidemainmenu=1',$msg);
	}
	...
	function unassign()
	{
		...
		{
			$msg=JText::sprintf('JL_ADMIN_TEAMSTAFF_CTRL_UNASSIGN',$nDeleted);
			$msg .= '<br/>'.$model->getError();
			$this->setRedirect('index.php?option=com_joomleague&controller=teamstaff&view=teamstaffs',$msg,'error');
		}
		else
		{
			$msg=JText::sprintf('JL_ADMIN_TEAMSTAFF_CTRL_UNASSIGN',$nDeleted);
