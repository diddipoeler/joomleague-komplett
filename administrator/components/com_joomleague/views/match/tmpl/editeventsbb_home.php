	...
	<table class='adminlist'>
		<thead>
			<tr>
				...
				<th style="text-align:left;"><?php echo JText::_( 'JL_ADMIN_MATCH_EEBB_PERSON' ); ?></th>
				<?php
			...
			for( $i=0 , $n = count( $this->homeRoster ); $i < $n; $i++ )
			{
					...
					<tr id="row<?php echo $i;?>">
						...
						foreach ( $this->events as $ev)
						{
                            ...
                            <input type="text" size="1" class="inputbox" name="event_sum_h_<?php echo $i.'_'.$tehp; ?>" value="<?php echo (($this->evbb[0]->event_sum > 0) ? $this->evbb[0]->event_sum : '' ); ?>" title="<?php echo JText::_( 'JL_ADMIN_MATCH_EE_VALUE_SUM' )?>" onchange="document.getElementById('cb_h<?php echo $i;?>').checked=true" />

                            <input type="text" size="2" class="inputbox" name="event_time_h_<?php echo $i.'_'.$tehp; ?>" value="<?php echo (($this->evbb[0]->event_time > 0) ? $this->evbb[0]->event_time : '' ); ?>" title="<?php echo JText::_( 'JL_ADMIN_MATCH_EE_TIME' )?>" onchange="document.getElementById('cb_h<?php echo $i;?>').checked=true" />

                            <input type="text" size="2" class="inputbox" name="notice_h_<?php echo $i.'_'.$tehp; ?>" value="<?php echo ((strlen($this->evbb[0]->notice) > 0) ? $this->evbb[0]->notice : '' ); ?>" title="<?php echo JText::_( 'JL_ADMIN_MATCH_EE_MATCH_NOTICE' )?>" onchange="document.getElementById('cb_h<?php echo $i;?>').checked=true" />
                            &nbsp;&nbsp;
                            </td>
