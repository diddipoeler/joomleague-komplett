...
JToolBarHelper::title(JText::_('JL_ADMIN_TREETONODES_TITLE'));
	if($isleafed==1)
	{
	JToolBarHelper::apply('saveshort', JText::_( 'JL_ADMIN_TREETONODES_SAVE_APPLY' ), false);
	JToolBarHelper::custom( 'removenode', 'delete.png', 'delete_f2.png', JText::_( 'JL_ADMIN_TREETONODES_DELETE_ALL' ), false );
	}
	elseif($isleafed)
	{
	JToolBarHelper::apply('saveshortleaf',JText::_( 'JL_ADMIN_TREETONODES_TEST_SHOW' ), false);
	if($isleafed==3)
	{
		JToolBarHelper::apply('savefinishleaf', JText::_( 'JL_ADMIN_TREETONODES_SAVE_LEAF' ), false);
	}
	JToolBarHelper::custom( 'removenode', 'delete.png', 'delete_f2.png', JText::_( 'JL_ADMIN_TREETONODES_DELETE' ), false );
	}
...
<form action="<?php echo $this->request_url; ?>" method="post" name="adminForm">
	<div id="editcell">
		<fieldset class="adminform">
			<legend><?php echo JText::sprintf('JL_ADMIN_TREETONODES_LEGEND','<i>'.$this->projectws->name.'</i>'); ?></legend>
<?php
$style=$this->style;
$path=$this->path;
...
if($isleafed==1)
{
	...
	{
			$link = JRoute::_( 'index.php?option=com_joomleague&controller=treetonode&task=edit&cid[]=' . $this->node[$j-1]->id );
			$ednode='<a href='. $link . '>';
		...
		echo $this->node[$j-1]->team_name;
			...
			$link3 = JRoute::_( 'index.php?option=com_joomleague&controller=treetomatch&view=treetomatchs&nid[]=' . $this->node[$j-1]->id );
			$match='<a href='. $link3 . '>';
			$match.=JHTML::_('image', 'administrator/components/com_joomleague/assets/images/matches.png','edit');
			$match.='</a>';
			echo $match;
			$link4 = JRoute::_( 'index.php?option=com_joomleague&controller=treetomatch&task=editlist&nid[]=' . $this->node[$j-1]->id );
