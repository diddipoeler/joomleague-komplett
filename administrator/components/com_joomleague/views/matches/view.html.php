...
class JoomleagueViewMatches extends JLGView
{
	...
	function _displayDefault($tpl)
	{
      ...
      /*
      if( $row->division_id ) {
				$divhomeid = $row->division_id;
        $row->divhomeid = $row->division_id;
				$row->divawayid = $row->division_id;
			} else {
				$row->divhomeid =0;
				$row->divawayid =0;
			}
    ...
    $divisions[]=JHTMLSelect::option('0',JText::_('JL_ADMIN_MATCHES_DIVISIONLIST'));
