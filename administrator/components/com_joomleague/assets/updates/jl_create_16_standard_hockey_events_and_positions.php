...
$updateDescription	= '<span style="color:green">Create standard hockey events,positions and position-events for use with JoomLeague&nbsp;v1.6</span>';
$excludeFile		= 'false';
...
function addSportsType()
{
	...
	echo JText::sprintf	(	'Adding the sports-type [%1$s] to table [%2$s] if it does not exist yet!',
							'<strong>'.'JL_S_HOCKEY'.'</strong>',
							'<strong>'.'#__joomleague_sports_type'.'</strong>'
						);

	$query="SELECT id FROM #__joomleague_sports_type WHERE name='JL_S_HOCKEY'";
	$db->setQuery($query);
	if (!$dbresult=$db->loadObject())
	{
		// Add new sportstype Soccer to #__joomleague_sports_type
		$queryAdd="INSERT INTO #__joomleague_sports_type (`name`) VALUES ('JL_S_HOCKEY')";
...
function addStandardsForHockey()
{
	...
	if ('AddEvents' == 'AddEvents')
	{
		...
		$query				= "SELECT * FROM #__joomleague_sports_type WHERE name='JL_S_HOCKEY'"; $db->setQuery($query); $sports_type=$db->loadObject();

		$newEventName='JL_E_GOAL';
		$newEventIcon='media/com_joomleague/event_icons/hockey/goal.png';
		...
		$newEventName='JL_E_GREEN_CARD';
		$newEventIcon='media/com_joomleague/event_icons/hockey/green_card.png';
		$query=build_SelectQuery('eventtypes',$newEventName); $db->setQuery($query);
		if (!$object=$db->loadObject())
		{
		...
		$newEventIcon='media/com_joomleague/event_icons/hockey/yellow_card.png';
		$query=build_SelectQuery('eventtypes',$newEventName); $db->setQuery($query);
		if (!$object=$db->loadObject())
		{
		...
		$newEventIcon='media/com_joomleague/event_icons/hockey/red_card.png';
		$query=build_SelectQuery('eventtypes',$newEventName); $db->setQuery($query);
		if (!$object=$db->loadObject())
		{
		...
		$newEventIcon='media/com_joomleague/event_icons/penalty_goal.png';
		$query=build_SelectQuery('eventtypes',$newEventName); $db->setQuery($query);
		if (!$object=$db->loadObject())
		{
		...
		$newEventIcon='media/com_joomleague/event_icons/injured.gif';
		$query=build_SelectQuery('eventtypes',$newEventName); $db->setQuery($query);
		if (!$object=$db->loadObject())
		{
	...
	if ('AddPositions' == 'AddPositions')
	{
		...
		if ('AddGeneralRefereesPositions' == 'AddGeneralRefereesPositions')
		{
			...
			if ('AddGeneralRefereesChildPositions' == 'AddGeneralRefereesChildPositions')
			{
				...
				$newPosName='JL_F_MAIN_REFEREE'; $newPosSwitch='persontype'; $newPosParent=$ParentID; $newPosContent='3';
				$query=build_SelectQuery('position',$newPosName); $db->setQuery($query);
				if (!$object=$db->loadObject())
				{
				...
				// New Child position for RefereePositions
				$newPosName='JL_F_THIRD_OFFICIAL'; $newPosSwitch='persontype'; $newPosParent=$ParentID; $newPosContent='3';
				$query=build_SelectQuery('position',$newPosName); $db->setQuery($query);
				if (!$object=$db->loadObject())
				{
				...
				// New Child position for RefereePositions
				$newPosName='JL_F_VIDEO_UMPIRE'; $newPosSwitch='persontype'; $newPosParent=$ParentID; $newPosContent='3';
				$query=build_SelectQuery('position',$newPosName); $db->setQuery($query);
				if (!$object=$db->loadObject())
				{
